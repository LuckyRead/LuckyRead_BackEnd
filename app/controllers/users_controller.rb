  ...
  def best
    ...
    User.bestuser(5).each do |tUser|
      tUser.each do |usern, talk, name, photo|
        ...
        if photo.nil?
          @hash = {username: @user.username, name: @user.name, lastname: @user.lastname, profile_photo: Photo.find(24).base64_image}
        else
          @hash = {username: @user.username, name: @user.name, lastname: @user.lastname, profile_photo: Photo.find_by(id: @user.photos_id).base64_image}
        end
        @array.push(@hash)
      end
    end
