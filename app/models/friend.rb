...
class Friend < ApplicationRecord
    ...
    def self.userfolloweresp(id, date) #return news followers of a specific user
        dateinit = date + ' 00:00:00'
        dateend = date + ' 23:59:59'
        return Friend.where("friends.followed = ? and friends.created_at >= ? and friends.created_at <= ?", id, dateinit, dateend).group(:followed).count()
    end 
end
